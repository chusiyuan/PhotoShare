...
class PhoteShareTest(unittest.TestCase):
    def setUp(self):
    ...
    def test_reg_logout_login(self):
        assert self.register("hello1", "world1").status_code == 200
        assert b'-hello1' in self.app.open('/').data
        self.logout()
        assert b'-hello1' not in self.app.open('/').data
        self.login("hello1", "world1")
        assert b'-hello1' in self.app.open('/').data
